...
def partition_tickers(tickers: list[str], chunk_size: int) -> list[list[str]]:
    """Split ticker list into roughly equal chunks for parallel processing."""
    if not tickers:
        return []
    n_chunks = max(1, math.ceil(len(tickers) / chunk_size))
    per_chunk = math.ceil(len(tickers) / n_chunks)
    return [tickers[i : i + per_chunk] for i in range(0, len(tickers), per_chunk)]


def get_sp500_tickers() -> list[str]:
    """Return a static list of S&P 500 tickers (avoids web scraping dependency)."""
    return [
